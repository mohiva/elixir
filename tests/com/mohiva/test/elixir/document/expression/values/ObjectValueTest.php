 ...
 */
class ObjectValueTest extends AbstractValueTest {

	/**
	 * Test if the constructor throws an `UnexpectedValueException` if the given value isn't a object value.
	 *
	 * @expectedException \com\mohiva\common\exceptions\UnexpectedValueException
	 */
	public function testConstructorThrowsExceptionIfValueIsNotObject() {
	...
	/**
	 * Test if the `getByProperty` method throws an `UnexpectedValueException` if a property doesn't exists.
	 *
	 * @expectedException \com\mohiva\common\exceptions\UnexpectedValueException
	 */
	public function testGetByPropertyThrowsException() {
	...
	/**
	 * Test if the `getByProperty` method returns a new `StringValue` object.
	 */
	public function testGetByPropertyReturnsNewValue() {

		$value = new stdClass();
		$value->test = 'a string';
		$value = new ObjectValue($value, $this->valueContext, $this->config);

		$this->assertInstanceOf('\com\mohiva\elixir\document\expression\values\StringValue',
			$value->getByProperty('test')
		);
	}
}
